...
} from '@osac/ui-components/api/v1/private/external-ip-pools';
...
const ExternalIpPoolForm = ({ pool }: { pool?: ExternalIPPool }) => {
  ...
  const isEdit = !!pool;
  const { mutateAsync: createPool, error: createError } = useCreateExternalIPPool();
  const { mutateAsync: updatePool, error: updateError } = useUpdateExternalIPPool();
  ...
  const onSubmit = async (values: ExternalIpPoolFormValues) => {
    try {
      if (pool) {
        await updatePool({
          id: pool.id,
          version: pool.metadata?.version ?? 0,
...
export const ExternalIpPoolFormPage = () => {
  ...
  const { data, isLoading, error } = usePrivateExternalIPPool(id ?? '');
  ...
  return <ExternalIpPoolForm pool={data} />;
};
